...
export default function Task() {
    ...
    const removeShowTodoList = (id) => {
        setUUid(id)
        dispatch({
            type: 'COUNT_CYCLES', payload: state.cycles.length
        })
    ...
    useEffect(() => {
        dispatch({
            type: 'DONE_TODO', payload: {
                id: uuid, cyclesNumber: state.cyclesNumber, date: new Date().getDate()
            }
        })
        dispatch({
        ...
        dispatch({
            type: 'REMOVE_SHOW_TODO', payload: []
        })

        dispatch({
            type: 'COUNT_CYCLES', payload: 0
        })
                    ...
                    <div>
                        ...
                        <div className="task--cycleIcon">
                            {state.cycles.map((item) => {
                                return <div key={item.id}><Circle size={12} bg="darkBlue"
